...
hi_u8 hcc_config_frame(oal_netbuf_head_stru *head)
{
    ...
    if (!oal_netbuf_list_empty(head)) {
        netbuf = oal_netbuf_head_next(head);
        hi_u8* hcc_hdr = (hi_u8*)oal_netbuf_data(netbuf);
        frw_hcc_extend_hdr_stru* ext_hdr = (frw_hcc_extend_hdr_stru*)(hcc_hdr + HCC_HDR_LEN + WLAN_MAX_MAC_HDR_LEN + HI_MAX_DEV_CB_LEN);
