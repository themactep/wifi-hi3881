...
static const wal_wid_op_stru g_ast_board_wid_op[] = {
    ...
    {WLAN_CFGID_AMSDU_TX_ON,       HI_FALSE,  {0},   HI_NULL,              hmac_config_set_amsdu_tx_on},
#endif
#endif
    ...
    while (us_req_idx < us_msg_len) {
        ...
        if (oal_unlikely(us_rsp_idx + sizeof(wal_msg_hdr_stru) > HMAC_RSP_MSG_MAX_LEN)) {
            oam_error_log3(mac_vap->vap_id, OAM_SF_ANY,
                "{wal_config_process_write::us_cfgid:%d reponse msg len:%u over limit:%u}", us_cfgid,
    ...
    if (request_address) {
        ...
        if (rsp_msg_tmp == NULL) {
            ...
            if (wal_set_msg_response_by_addr(request_address, NULL, HI_ERR_CODE_PTR_NULL, rsp_toal_len) != HI_SUCCESS) {
                oam_error_log0(0, OAM_SF_ANY, "{wal_set_msg_response::did't found the request msg!");
            }
        } else {
...
hi_u32 wal_config_process_pkt(frw_event_mem_stru *event_mem)
{
    ...
    switch (msg->msg_hdr.msg_type) {
        ...
        default:
            oam_warning_log1(mac_vap->vap_id, OAM_SF_ANY,
                "{wal_config_process_pkt::msg_type Err=%d}", msg->msg_hdr.msg_type);
    ...
    if (oal_unlikely(rsp_toal_len > HMAC_RSP_MSG_MAX_LEN)) {
        oam_error_log1(mac_vap->vap_id, OAM_SF_ANY, "{wal_config_process_pkt::invaild response len %u}", rsp_toal_len);
        return HI_FAIL;
    }
...
hi_u32 hmac_event_config_syn(frw_event_mem_stru *event_mem)
{
    ...
    oam_info_log1(event_hdr->vap_id, OAM_SF_CFG,
                  "{hmac_event_config_syn::a dmac config syn event occur, cfg_id=%d.}", dmac2hmac_msg->syn_id);
    /* 获取dmac vap */
    ...
    if (ret != HI_SUCCESS) {
        oam_warning_log2(event_hdr->vap_id, OAM_SF_CFG,
                         "{hmac_event_config_syn::p_set_func failed, ul_ret=%d en_syn_id=%d.", ret,
                         dmac2hmac_msg->syn_id);
        return ret;
    }
...
/*****************************************************************************
 功能描述  : hmac接收dmac数据事件
*****************************************************************************/
hi_u32 hmac_from_dmac_rx_data_handle(frw_event_mem_stru *event_mem, oal_netbuf_stru *netbuf,
                                     hi_u16 netbuf_num)
{
