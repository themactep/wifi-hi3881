 ...
 * Description: HMAC module manages the source file defined by the frame operation function.
 * Author: Hisilicon
 * Create: 2018-08-04
 */
        ...
        switch (action_args->action) {
            ...
            default:
                oam_warning_log1(hmac_vap->base_vap->vap_id, OAM_SF_TX,
                                 "{hmac_mgmt_tx_action::invalid ba type[%d].}", action_args->action);
                return HI_FAIL;
        }
    } else {
        oam_info_log1(hmac_vap->base_vap->vap_id, OAM_SF_TX,
                      "{hmac_mgmt_tx_action::invalid ba type[%d].}", action_args->category);
    }
    return HI_SUCCESS;
}
...
hi_u32 hmac_rx_process_mgmt_event(frw_event_mem_stru *event_mem)
{
    ...
    if (mac_find_vendor_action(MAC_WLAN_OUI_HUAWEI, MAC_OUITYPE_ANY,
                               data, rx_ctrl->us_frame_len - rx_ctrl->mac_header_len) != HI_FALSE) {
        /* 处理接收到的ANY管理帧 */
    ...
    mac_vap_stru *mac_vap, const hi_u8 *mac_addr, hi_u16 us_err_code, hmac_vap_stru *hmac_vap)
{
    ...
    if (disasoc_deauth_event->event == DMAC_WLAN_CRX_DEAUTH) {
        ...
        hmac_mgmt_send_deauth_frame(up_vap2, mac_addr, WLAN_MAC_ADDR_LEN,
...
hi_u32 hmac_proc_disasoc_misc_event(frw_event_mem_stru *event_mem)
{
    ...
    if (is_ap(hmac_vap->base_vap)) {
        ...
        /* DTS2015052810869 STA发现AP信道修改，则删除用户，而不发送去认证帧 */
