    ...
    hi_u8 *puc_buffer, hi_u16 us_frm_type, hi_u8 *puc_ie_len)
{
    ...
    if (us_frm_type == WLAN_FC0_SUBTYPE_PROBE_RSP) {
        ...
        auc_oui[2] = (hi_u8)MAC_WLAN_OUI_HUAWEI2; /* auc_oui[2]赋值为MAC_WLAN_OUI_HUAWEI2 */
                                                  ...
                                                  const hi_u8 *puc_elements, hi_u8 element_len)
{
    ...
    puc_body[MAC_ACTION_OUI_POS + 1] = MAC_WLAN_OUI_HUAWEI1; /* vendor OUI byte(1+1)设置 */
    puc_body[MAC_ACTION_OUI_POS + 2] = MAC_WLAN_OUI_HUAWEI2; /* vendor OUI byte(1+2)设置 */
    /* 设置action类型和子类型 */
...
hi_void mac_set_vendor_hisi_ie(hi_u8 *puc_buffer, hi_u8 *puc_ie_len)
{
    ...
    vendor_ie->auc_oui[0] = (hi_u8)((MAC_HUAWEI_VENDER_IE >> 16) & 0xff); /* oui[0]由最低HW IE次最高16 bit获取 */
    vendor_ie->auc_oui[1] = (hi_u8)((MAC_HUAWEI_VENDER_IE >> 8) & 0xff);  /* oui[1]由最低HW IE次低8 bit获取 */
    vendor_ie->auc_oui[2] = (hi_u8)((MAC_HUAWEI_VENDER_IE) & 0xff);       /* oui[2]由最低HW IE最低8 bit获取 */
