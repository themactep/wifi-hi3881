...
static hi_u32 wal_ioctl_get_iwrange(oal_net_device_stru *netdev, oal_iw_request_info_stru *info,
                                    oal_iw_point_stru *param, hi_s8 *pc_extra);
static hi_u32 wal_ioctl_get_param(oal_net_device_stru *netdev, oal_iw_request_info_stru *info,
                                  hi_void *iw, hi_s8 *pc_extra);
static hi_u32 wal_ioctl_set_param(oal_net_device_stru *netdev, oal_iw_request_info_stru *info,
                                  hi_void *iw, hi_s8 *pc_extra);
static hi_u32 wal_ioctl_get_iwname(oal_net_device_stru *netdev, oal_iw_request_info_stru *info,
                                   hi_s8* pc_name, hi_s8* pc_extra);
static hi_s32 wal_ioctl_set_wme_params(oal_net_device_stru *netdev, oal_iw_request_info_stru *info,
                                       hi_void *iw, hi_s8 *pc_extra);
static hi_u32 wal_ioctl_get_wme_params(oal_net_device_stru *netdev, oal_iw_request_info_stru *info,
                                       hi_void *iw, hi_s8 *pc_extra);
static hi_u32 wal_ioctl_setcountry(oal_net_device_stru *netdev, oal_iw_request_info_stru *info,
                                   hi_void *w, hi_s8 *pc_extra);
static hi_u32 wal_ioctl_getcountry(oal_net_device_stru *netdev, oal_iw_request_info_stru *info,
                                   hi_void *w, hi_s8 *pc_extra);
...
hi_u32 wal_cfg_vap_h2d_event(oal_net_device_stru *netdev)
{
    ...
    ***************************************************************************/
    ...
    /* 发送消息 */
    ret = wal_send_cfg_event(netdev_cfg,
                             WAL_MSG_TYPE_WRITE,
...
static hi_u32 wal_host_dev_config(const oal_net_device_stru *netdev, wlan_cfgid_enum_uint16 wid)
{
    ...
    wdev = oal_netdevice_wdev(netdev);
...
hi_u32 wal_ioctl_set_mode(oal_net_device_stru *netdev, hi_char *pc_param)
{
    ...
    ***************************************************************************/
    ...
    oam_info_log3(0, OAM_SF_CFG, "{wal_ioctl_set_mode::protocol[%d],band[%d],bandwidth[%d]!}\r\n",
                  mode_param->protocol, mode_param->band, mode_param->en_bandwidth);
...
hi_u32  wal_ioctl_set_essid(oal_net_device_stru *netdev, hi_char *pc_param)
{
    ...
    || (mac_vap->vap_mode == WLAN_VAP_MODE_MESH)
        ...
        if (0 != (OAL_IFF_RUNNING & oal_netdevice_flags(netdev))) {
            oam_error_log1(mac_vap->vap_id, 0,
...
hi_u32  wal_ioctl_set_freq(oal_net_device_stru *netdev, hi_char *pc_param)
{
    ...
    if (0 != (OAL_IFF_RUNNING & oal_netdevice_flags(netdev))) {
        oam_error_log1(0, 0,
            "{wal_ioctl_set_freq::device is busy, please down it first %d!}\r\n", oal_netdevice_flags(netdev));
        return HI_ERR_WIFI_WAL_BUSY;
    }
    ...
    if (ret != HI_SUCCESS) {
        oam_warning_log1(0, 0,
            "{wal_ioctl_set_freq::wal_get_cmd_one_arg vap name return err_code %u!}\r\n", ret);
        return ret;
    }
...
*****************************************************************************/
static hi_u32  wal_ioctl_get_apaddr(
...
*****************************************************************************/
static hi_u32  wal_ioctl_get_iwrate(
                oal_net_device_stru         *netdev,
                oal_iw_request_info_stru    *info,
                oal_iw_param_stru           *param,
                hi_s8                    *pc_extra)
{
...
*****************************************************************************/
static hi_u32  wal_ioctl_get_iwsense(
                oal_net_device_stru         *netdev,
                oal_iw_request_info_stru    *info,
                oal_iw_param_stru           *param,
                hi_s8                    *pc_extra)
...
*****************************************************************************/
static hi_u32  wal_ioctl_get_rtsthres(
                oal_net_device_stru         *netdev,
                oal_iw_request_info_stru    *info,
                oal_iw_param_stru           *param,
                hi_s8                    *pc_extra)
...
*****************************************************************************/
static hi_u32  wal_ioctl_get_fragthres(
                oal_net_device_stru         *netdev,
                oal_iw_request_info_stru    *info,
                oal_iw_param_stru           *param,
                hi_s8                    *pc_extra)
...
*****************************************************************************/
static hi_u32  wal_ioctl_get_iwencode(
                oal_net_device_stru         *netdev,
                oal_iw_request_info_stru    *info,
...
*****************************************************************************/
static hi_u32  wal_ioctl_get_iwrange(
                oal_net_device_stru         *netdev,
                oal_iw_request_info_stru    *info,
                oal_iw_point_stru           *param,
                hi_s8                    *pc_extra)
    ...
    hi_void *iw, hi_s8 *pc_extra)
{
    ...
    ***************************************************************************/
    ...
    ret = wal_send_cfg_event(netdev,
                             WAL_MSG_TYPE_WRITE,
                             WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(wal_msg_wmm_stru),
                             (hi_u8 *)&write_msg,
                             HI_FALSE,
                             HI_NULL);
    if (oal_unlikely(ret != HI_SUCCESS)) {
                                       ...
                                       oal_iwreq_data_union *wrqu, hi_s8 *pc_extra)
{
    ...
    ***************************************************************************/
    ...
    if (pst_query_rsp_msg->us_len >= sizeof(oal_netbuf_stru)) {
        /* 获取hmac保存的netbuf指针 */
        memcpy_s(&pst_response_netbuf, sizeof(oal_netbuf_stru),
        ...
        oal_print_hex_dump((hi_u8 *)pst_rsp_msg->auc_msg_data, pst_query_rsp_msg->us_len,
            32, "query msg: "); /* group size 32 */
        l_ret = -OAL_EINVAL;
    }
...
static hi_s32 wal_config_mac_filter(oal_net_device_stru *net_dev, hi_char *pc_command)
{
    ...
    if (ul_mac_mode > 2) {  /* mac mode 2 */
        oam_warning_log4(0, 0, "{wal_config_mac_filter::invalid MAC_MODE[%c%c%c%c]!}\r\n",
            (hi_u8)parsed_command[0], (hi_u8)parsed_command[1],     /* command 0/1 */
    ...
    for (ul_i = 0; ul_i < ul_mac_cnt; ul_i++) {
        ...
        ***************************************************************************/
        ...
        if ((l_ret != HI_SUCCESS)  || (pst_rsp_msg == HI_NULL)) {
            oam_error_log1(0, 0,
                "{wal_config_mac_filter:: wal_send_cfg_event return err code %d!}\r\n", l_ret);
            return l_ret;
        }
                                        ...
                                        oal_iwreq_data_union *wrqu, hi_s8 *pc_extra)
{
    ...
    /* 3  解析MAC_MODE */
    ul_ret = wal_get_parameter_from_cmd(pc_parse_command, parsed_command, "MAC_MODE=",
    ...
    ul_mac_mode = (hi_u32)oal_atoi(parsed_command);
    if (ul_mac_mode > 2) {  /* mac mode 2 */
        oam_warning_log4(0, 0, "{wal_ioctl_set_mac_filters::invalid MAC_MODE[%c%c%c%c]!}",
            (hi_u8)parsed_command[0], (hi_u8)parsed_command[1],     /* command 0/1 */
            (hi_u8)parsed_command[2], (hi_u8)parsed_command[3]);    /* command 2/3 */
        oal_free(pc_command);
        return -OAL_EINVAL;
                                      ...
                                      oal_iwreq_data_union *wrqu, hi_s8 *pc_extra)
{
    ...
    /* 3.   解析参数 */
    /* 3.1  解析ASCII_CMD */
    ul_ret = wal_get_parameter_from_cmd(pc_parse_command, ac_parse_command, "ASCII_CMD=",
                                        &off_set, WAL_IOCTL_PRIV_SUBCMD_MAX_LEN);
    if (ul_ret != HI_SUCCESS) {
        oam_warning_log1(0, 0,
    ...
    pc_parse_command += off_set;
    ul_ret = wal_get_parameter_from_cmd(pc_parse_command, ac_parse_command, "CHANNEL=",
                                        &off_set, WAL_IOCTL_PRIV_SUBCMD_MAX_LEN);
    if (ul_ret != HI_SUCCESS) {
        oam_warning_log1(0, 0,
    ...
    /* 3.3  解析MAX_SCB */
    pc_parse_command += off_set;
    ul_ret = wal_get_parameter_from_cmd(pc_parse_command, ac_parse_command, "MAX_SCB=",
        &off_set, WAL_IOCTL_PRIV_SUBCMD_MAX_LEN);
    if (ul_ret != HI_SUCCESS) {
        oam_warning_log1(0, 0,
    ...
    oam_warning_log1(0, 0,
        "{wal_ioctl_set_ap_config:: 1131_debug:: ul_ap_max_user = [%d]!}\r\n", g_st_ap_config_info.ap_max_user);
...
hi_u32 wal_set_mac_to_mib(oal_net_device_stru *netdev)
{
    ...
    if (memcpy_s(param->auc_station_id, WLAN_MAC_ADDR_LEN, netdev->dev_addr, WLAN_MAC_ADDR_LEN) != EOK) {
                                ...
                                en_app_ie_type_uint8 type)
{
    ...
    ***************************************************************************/
    ...
    hi_u32 ret = wal_send_cfg_event(netdev, WAL_MSG_TYPE_WRITE,
                                    WAL_MSG_WRITE_MSG_HDR_LENGTH + sizeof(wps_p2p_ie),
    ...
    ***************************************************************************/
    ...
    if ((ret != HI_SUCCESS) || (rsp_msg == HI_NULL)) {
        oam_error_log1(0, OAM_SF_P2P,
            "{wal_ioctl_set_p2p_noa:: wal_alloc_cfg_event return err code %u!}\r\n", ret);
        return ret;
    }
    ...
    if (ret != HI_SUCCESS) {
        oam_error_log1(0, OAM_SF_P2P,
            "{wal_ioctl_set_p2p_noa::wal_send_cfg_event return err code:  [%d]!}\r\n", ret);
        return ret;
    }
...
hi_u32 wal_ioctl_set_p2p_ops(oal_net_device_stru *netdev, const mac_cfg_p2p_ops_param_stru *p2p_ops_param)
{
    ...
    ***************************************************************************/
    ...
    if ((ret != HI_SUCCESS) || (rsp_msg == HI_NULL)) {
        oam_error_log1(0, OAM_SF_P2P,
            "{wal_ioctl_set_p2p_ops:: wal_alloc_cfg_event return err code %u!}\r\n", ret);
        return ret;
    }
    ...
    if (ret != HI_SUCCESS) {
        oam_error_log1(0, OAM_SF_P2P,
            "{wal_ioctl_set_p2p_ops::wal_send_cfg_event return err code:[%u]!}\r\n", ret);
        return ret;
    }
...
static hi_u32 wal_ioctl_reduce_sar(oal_net_device_stru *netdev, hi_u8 tx_power)
{
    ...
    oam_warning_log1(0, 0,
        "wal_ioctl_reduce_sar::supplicant set tx_power[%d] for reduce SAR purpose.\r\n", tx_power);
...
hi_u32 wal_get_vap_p2p_mode(const oal_wireless_dev *wdev,
...
hi_u32 wal_init_wlan_vap(oal_net_device_stru *netdev)
{
    ...
    if ((wdev == HI_NULL) || (netdev_cfg == HI_NULL)) {
        oam_warning_log2(0, 0, "{wal_init_wlan_vap::wdev[%p]/netdev_cfg[%p] is null}",
            (uintptr_t)wdev, (uintptr_t)netdev_cfg);
        return HI_ERR_CODE_PTR_NULL;
    }
...
*****************************************************************************/
#ifdef _PRE_WLAN_FEATURE_P2P
hi_u32 wal_android_p2p_cmd_exec(oal_net_device_stru *netdev, hi_u8 *puc_command, hi_u32 len)
{
    if (memcmp(puc_command, CMD_P2P_SET_NOA, strlen(CMD_P2P_SET_NOA)) == 0) {
        ...
        if ((skip + sizeof(p2p_noa_param)) > len) {
            oam_error_log2(0, 0, "{wal_android_priv_cmd_exec::CMD_P2P_SET_NOA param len is [%d], need %d.}",
                len, skip + sizeof(p2p_noa_param));
            return HI_FAIL;
        }
        if (memcpy_s(&p2p_noa_param, sizeof(mac_cfg_p2p_noa_param_stru),
            puc_command + skip, sizeof(mac_cfg_p2p_noa_param_stru)) != EOK) {
            oam_error_log0(0, 0, "wal_android_priv_cmd_exec: copy noa param failed!");
            return HI_FAIL;
        }
        ...
        if ((skip + sizeof(p2p_ops_param)) > len) {
            oam_error_log2(0, 0, "{wal_android_priv_cmd_exec::CMD_P2P_SET_PS param len is %d, need %d.}",
                len, skip + sizeof(p2p_ops_param));
            return HI_FAIL;
        }
        if (memcpy_s(&p2p_ops_param, sizeof(mac_cfg_p2p_ops_param_stru),
            puc_command + skip, sizeof(mac_cfg_p2p_ops_param_stru)) != EOK) {
            oam_error_log0(0, 0, "wal_android_priv_cmd_exec: copy ops param failed!");
            return HI_FAIL;
        }
...
/*****************************************************************************
 ...
 返 回 值  : 统计结果指针
*****************************************************************************/
hi_u32 wal_android_priv_cmd_exec(oal_net_device_stru *netdev, hi_u8 *puc_command, hi_u32 len)
{
    if (0 == memcmp(puc_command, CMD_COUNTRY, strlen(CMD_COUNTRY))) {
        ...
        if (len < (strlen((hi_s8 *)CMD_COUNTRY) + 3)) { /* 3: 偏移量 */
            oam_warning_log0(0, 0, "{wal_android_priv_cmd_exec::country cn len error.}");
            return HI_FAIL;
        }
        ...
        if (memcpy_s(auc_country_code, sizeof(auc_country_code), country_code, 2) != EOK) { /* 复制长度为2 */
            oam_error_log0(0, 0, "wal_android_priv_cmd_exec: copy country code failed!");
            return HI_FAIL;
        }
        ...
        if (wal_ioctl_reduce_sar(netdev, txpwr) != HI_SUCCESS) {
            oam_warning_log0(0, 0, "{wal_android_priv_cmd_exec::return err code [%u]!}");
...
#ifdef _PRE_WLAN_FEATURE_P2P
    /* p2p相关配置命令执行 */
    return wal_android_p2p_cmd_exec(netdev, puc_command, len);
#endif
    return HI_SUCCESS;
}
/*****************************************************************************
...
{
    wal_android_wifi_priv_cmd_stru  priv_cmd    = {0};
    ...
    if (oal_copy_from_user((hi_u8 *)&priv_cmd, sizeof(wal_android_wifi_priv_cmd_stru),
        ifr->ifr_data, sizeof(wal_android_wifi_priv_cmd_stru)) != 0) {
        return HI_FAIL;
    }

    if ((priv_cmd.l_total_len > MAX_PRIV_CMD_SIZE) || (priv_cmd.l_total_len < 0)) {
        oam_error_log1(0, 0, "{wal_android_priv_cmd::priavte command len:%d too long}", priv_cmd.l_total_len);
        return HI_FAIL;
    }
    ...
    if (puc_command == HI_NULL) {
        oam_error_log0(0, 0, "{wal_android_priv_cmd::mem alloc failed.}");
        return HI_FAIL;
    }
    /* 拷贝wpa_supplicant 命令到内核态中 */
    if (memset_s(puc_command, (hi_u32)(priv_cmd.l_total_len + 1), 0, (hi_u32)(priv_cmd.l_total_len + 1)) != EOK) {
        oam_error_log0(0, 0, "wal_android_priv_cmd: mem safe function err!");
    ...
    if (ret != 0) {
        oam_error_log1(0, 0, "{wal_android_priv_cmd::oal_copy_from_user: err code [%u] }", ret);
        oal_free(puc_command);
        return ret;
    }
    puc_command[priv_cmd.l_total_len] = '\0';
    oam_warning_log4(0, 0, "{wal_android_priv_cmd:: private cmd total_len:%d, used_len:%d, CMD:0x%x %x}",
        priv_cmd.l_total_len, priv_cmd.l_used_len, *((hi_u32*)puc_command), *((hi_u32*)(puc_command + 4))); /* 4偏移 */

    /* 执行命令字 */
    ret = wal_android_priv_cmd_exec(netdev, puc_command, (hi_u32)priv_cmd.l_total_len);
    oal_free(puc_command);
    return ret;
}
#endif
